...
export var HorizontalHeadersArea = AreaItem.inherit({
  ...
  _applyCustomStyles: function (options) {
    ...
    if (options.cellIndex === options.cellsCount - 1) {
      cssArray.push(
        (options.rtlEnabled ? "border-left:" : "border-right:") + "0px"
      );
    }
    if (
      cell.rowspan === rowsCount - options.rowIndex ||
      options.rowIndex + 1 === rowsCount
    ) {
    ...
    if (isDefined(cell.expanded)) {
      classArray.push(
        cell.expanded ? PIVOTGRID_EXPANDED_CLASS : PIVOTGRID_COLLAPSED_CLASS
      );
    }
  ...
  hasScroll: function () {
    var tableWidth = this._virtualContent
      ? this._virtualContentWidth
      : this._tableWidth;
  ...
  processScrollBarSpacing: function (scrollBarWidth) {
    scrollBarWidth =
      scrollBarWidth && this.option("useBetterScroller") ? 8 : scrollBarWidth;
    ...
    this._groupElement.toggleClass(
      PIVOTGRID_VERTICAL_SCROLL_CLASS,
      scrollBarWidth > 0
    );
  ...
  getScrollPath: function (offset) {
    ...
    each(tableElement.find("td"), function (_, td) {
      if (
        1 === td.colSpan &&
        td.offsetLeft <= offset &&
        td.offsetWidth + td.offsetLeft > offset
      ) {
        cell = td;
        return false;
      }
...
export var VerticalHeadersArea = HorizontalHeadersArea.inherit({
  ...
  hasScroll: function () {
    var tableHeight = this._virtualContent
      ? this._virtualContentHeight
      : this._tableHeight;
  ...
  _getRowClassNames: function (rowIndex, cell, rowClassNames) {
    if (
      (0 !== rowIndex) & cell.expanded &&
      -1 === inArray(PIVOTGRID_EXPAND_BORDER, rowClassNames)
    ) {
  ...
  updateColspans: function (columnCount) {
    ...
    for (var i = 0; i < rows.length; i++) {
      for (var j = 0; j < rows[i].cells.length; j++) {
        ...
        columnOffsetResetIndexes[i + rowSpan] =
